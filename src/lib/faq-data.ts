...
export const FAQ_DATA: Record<AlgorithmType, FaqContent> = {
    bubbleSort: {
        title: "Bubble Sort",
        faqs: [
            {
    ...
    introSort: {
        ...
        faqs: [
            {
                ...
                answer: "Shell Sort is an in-place comparison sort. It can be seen as either a generalization of sorting by exchange (bubble sort) or sorting by insertion (insertion sort). The method starts by sorting pairs of elements far apart from each other, then progressively reducing the gap between elements to be compared."
            },
            {
                question: "What is its time complexity?",
                answer: "The time complexity of Shell sort depends on the gap sequence used. For the gaps proposed by Donald Shell, the worst-case complexity is O(n^2). For other gap sequences, it can be as good as O(n log^2 n) or O(n^(3/2))."
            },
             {
     ...
     combSort: {
        ...
        faqs: [
            {
                question: "What is Comb Sort?",
                answer: "Comb Sort is a relatively simple sorting algorithm that improves on Bubble Sort. The basic idea is to eliminate 'turtles', or small values near the end of the list, since these slow down bubble sort drastically. It does this by using a gap larger than 1, which is then shrunk in each pass."
            },
            {
                question: "What is the time complexity of Comb Sort?",
                answer: "The average time complexity is O(n log n), and the worst-case is O(n^2), although the worst case is difficult to hit in practice. It's a significant improvement over Bubble Sort."
            },
             {
     ...
     cycleSort: {
        ...
        faqs: [
            {
                question: "What is Cycle Sort?",
                answer: "Cycle Sort is a comparison sorting algorithm that is theoretically optimal in terms of the total number of writes to the original array. It is an in-place sorting algorithm and is unstable."
            },
            {
                question: "How does Cycle Sort work?",
                answer: "It works by finding cycles of elements that need to be rotated to get them into their correct positions. It finds the correct position for an element and places it there, displacing another element, and it continues this process until all elements are in their correct positions."
            },
             {
